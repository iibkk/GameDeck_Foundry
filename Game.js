...
loginForm.addEventListener('submit', async (e) => {
  ...
  try {//(teacher login URL change here)
    ...
    if (response.ok) {
      ...
      // Save the Token generated by the backend in the browser's local storage
...
document.getElementById('studentForm').addEventListener('submit', async function (e) {
  ...
  try {
    // 🚨 修正小提示：注意检查你后端的端口！如果你的登录/注册在 3001，确保 session 接口的端口也对齐。
    // 这里保持你原本的 3000 端口发送请求
    ...
    if (response.ok) {
      ...
      // 🚨 核心终极修改：拼装完美的、能让 Unity 脚本直接抠出参数的接头网址！
      // 1. 将关键词从 room 改为 roomCode
      // 2. 传入全小写的 role=student (防止 Unity C# 大小写判断拦截)
      // 3. 传入 name=${encodeURIComponent(nickname)} 传递学生花名
